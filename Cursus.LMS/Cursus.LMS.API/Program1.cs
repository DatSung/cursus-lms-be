    ...
    public class Program1
    {
        ...
        private static void ApplyMigration(WebApplication app)
        {
            using (var scope = app.Services.CreateScope())
            {
                ...
                // Check if the database provider is relational
                var relationalDatabase = context.Database.IsRelational();

                if (relationalDatabase && context.Database.GetPendingMigrations().Any())
                {
